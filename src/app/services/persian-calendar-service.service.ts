...
export class PersianCalendarService {
  ...
  monthNames: string[] = [
    ...
    "بهمن",
    "اسفند"];
  strWeekDay: string | null = null;
  strMonth: string | null = null;
  day: number | any = null;
  month: number | null = null;
  year: number | null = null;
  ld: number | null = null;
  farsiDate: string | null = null;

  today: Date = new Date();

  gregorianYear : number | any = null;
  gregorianMonth: number | any = null;
  gregorianDate: number | any = null;
  WeekDay: number | any = null;
  ...
  toPersian(gregorianDate: any) {
    if (this.gregorianYear != null && (this.gregorianYear % 4) != 0)
      this.farsiDate = this.func1();
    else
      this.farsiDate = this.func2();
    this.strMonth = this.monthNames[Math.floor((this.month ? this.month : 1) - 1)];
    this.strWeekDay = this.weekDayNames[(this.WeekDay ? this.WeekDay : 1)  + 1];
  }

  func1(): string {
    this.day = this.buf1[(this.gregorianMonth ? this.gregorianMonth : 1) - 1] + this.gregorianDate;
    if (this.day > 79) {
